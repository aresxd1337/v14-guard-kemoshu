...
module.exports = {
    ...
    ws: false,
    run: async (kemo) => {
        let pic = await kemo.guild.fetchAuditLogs({ type: "CHANNEL_DELETE" }).then(a => a.entries.first());
        let logcuk = kemo.guild.channels.cache.get(settings.guild.log)
        let ibne = kemo.guild.members.cache.get(pic.executor.id)
        if (!pic || Date.now() - pic.createdTimestamp > 5000 || await full(pic.executor.id) || await rolec(pic.executor.id)) return;
        if (ibne.bannable) ibne.ban({ reason: `kemo Kanal Silme Koruması` });
        ytKapat(kemo.guild.id)
        if (logcuk) {
            logcuk.send({
                embeds: [new Discord.MessageEmbed().setFooter(`Kanalı kurmak için \`/kanalkur kanal: ${kemo.id}\``).setDescription(`
\`>\` ${pic.executor} \`(${pic.executor.id})\` kişisi tarafından kanal silindi!

\`>\` Kullanıcı : ${ibne.bannable ? "**Başarıyla yasaklandı!**" : "**Yasaklanamadı!**"}

\`>\` Silinen Rol : ${kemo.name} **(${kemo.id})**
                `)], content: `@everyone`
            })
        }
    },
