...
module.exports = {
    ...
    ws: false,
    run: async (kemo) => {
        let pic = await kemo.guild.fetchAuditLogs({ type: "ROLE_DELETE" }).then(a => a.entries.first());
        ...
        if (ibne.bannable) ibne.ban({ reason: `kemo Rol Silme Koruması` });
        ytKapat(kemo.guild.id)
        if (logcuk) {
            logcuk.send({
                embeds: [new Discord.MessageEmbed().setFooter(`Rolü kurmak için \`/rolkur rolid: ${kemo.id}\``).setDescription(`
